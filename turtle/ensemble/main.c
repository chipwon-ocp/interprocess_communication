...
main(void)
{
    ...
    printf("[M55HE] Build information: %s - %s\n", __DATE__, __TIME__);
    printf("[M55HE] CPU name: %s\n", CPU_NAME);
    printf("[M55HE] System clock: %d Hz\n", SystemCoreClock);
    printf("[M55HE] Enter main!!!\n");
    /* Create application main thread */
    BaseType_t xReturned = xTaskCreate(blinky_main, "turtle\\blinky", 96, NULL, configMAX_PRIORITIES-1, &task_handle_blinky);
    if (xReturned != pdPASS)
    {
        vTaskDelete(task_handle_blinky);
        printf("[M55HE] Abort main!!!\n");
        return -1;
    }
    xReturned = xTaskCreate(tty_main, "turtle\\tty", 160, NULL, configMAX_PRIORITIES-1, &task_handle_tty);
    if (xReturned != pdPASS)
    {
    ...
    xReturned = xTaskCreate(msg_comm_main, "turtle\\msg_comm", 256, NULL, 1, &task_handle_msg_comm);
    if (xReturned != pdPASS)
    {
